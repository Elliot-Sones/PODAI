...
import { sendEmail, createInvitation, stepInvitations, RawInvite } from './invites.js';
...
program
  ...
  .action(async (csvFile: string, opts) => {
    try {
      ...
      csvData = csvData.filter((row: any) => row.priority === opts.priority);
      for (const record of csvData) {
        const invitation = await createInvitation(supabase, record as RawInvite);
...
program
  .command('populate')
  .description('Auto-populate podcasts from curated catalog by category.')
  .option('--category <category>', 'Category to populate (technology, health, business).')
  .option('--all', 'Populate all categories.')
  .option('--owner <owner>', 'Owner ID for created podcasts.', DEFAULT_OWNER)
  .action(async (opts) => {
    const categories = opts.all ? getAvailableCategories() : opts.category ? [opts.category] : [];
    if (categories.length === 0) {
      term.red('Please specify --category <name> or --all\n');
      term('Available categories: ').green(getAvailableCategories().join(', '))('\n');
      return;
    }
    ...
    for (const category of categories) {
      ...
      if (!catRow) {
        term.red(`Category "${category}" not found in database. Run the categories migration first.\n`);
        continue;
      }

      for (const podcastEntry of podcasts) {
        try {
          // Check if already exists
          try {
            const existing = await GetPodcasts(supabase);
            const match = existing.find((p) => p.rssUrl === podcastEntry.rssUrl);
            if (match) {
              term.yellow(`  Skipping (exists): ${podcastEntry.name}\n`);
              // Ensure category mapping exists
              await supabase.from('PodcastCategories').upsert(
                { podcast: match.id, category: catRow.id },
          ...
          term.blue(`  Importing: ${podcastEntry.name}...\n`);
          const podcast = await Ingest({
            supabase,
            podcastUrl: podcastEntry.rssUrl,
            owner: opts.owner,
          });
          term.green(`  Imported: ${podcast.slug} (${podcast.Episodes?.length || 0} episodes)\n`);
          ...
          term.red(`  Error importing ${podcastEntry.name}: ${err}\n`);
        }
      }
    }
    term.green('\nPopulation complete!\n');
