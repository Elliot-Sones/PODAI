...
import { CitationMoment } from '@/components/CitationMoment';
          ...
          components={{
            ...
            a({ children, href }) {
              ...
              if (href?.startsWith('/?suggest')) {
                ...
                return <CitationMoment time={time} isCurrentEpisode={true} />;
              } else if (href?.includes('?seek=')) {
                // Cross-episode citation link: /podcast/foo/episode/bar?seek=123.45
                const [episodePath, seekParam] = href.split('?seek=');
                const time = parseFloat(seekParam);
                return (
                  <div>
                    <MagicLink href={episodePath} uuid={uuid} embed={embed}>
                      {children}
                    </MagicLink>
                    <CitationMoment time={time} episodeLink={episodePath} isCurrentEpisode={false} />
                  </div>
                );
              } else if (href) {
