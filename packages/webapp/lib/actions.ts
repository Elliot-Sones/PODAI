...
import {
  ...
  UploadLargeFile,
  Category,
  GetPopularTopics as GetPopularTopicsUtil,
  SearchTopics as SearchTopicsUtil,
  SearchTopicChunks as SearchTopicChunksUtil,
  TopicWithCount,
  TopicPodcastResult,
  searchPodcasts as searchPodcastsUtil,
  PodcastSearchResult,
  lookupPodcast,
  extractAppleId,
  CategorizePodcast,
  AssignPodcastCategory,
} from 'podverse-utils';
...
export async function assignPodcastToUser(podcastId: number, userId: string, activationCode: string): Promise<void> {
  ...
  console.log(`Podcast ${podcastId} ownership transferred to user ${userId}`);
}
...
export async function getPodcastsByCategory(categorySlug: string) {
  const supabase = await getSupabaseClient();
  ...
  const { data: podcastCategories } = await supabase
    .from('PodcastCategories')
    .select('podcast')
    .eq('category', category.id);

  const podcastIds = (podcastCategories || []).map((pc: { podcast: number }) => pc.podcast);
...
export async function getPopularTopics(): Promise<TopicWithCount[]> {
  const supabase = await getSupabaseClient();
  return GetPopularTopicsUtil(supabase);
}

/** Search topics semantically. */
export async function searchTopics(query: string): Promise<TopicWithCount[]> {
  const supabase = await getSupabaseClient();
  return SearchTopicsUtil(supabase, query);
}

/** Search transcript chunks for a query, grouped by podcast/episode. */
export async function searchTopicChunks(query: string): Promise<TopicPodcastResult[]> {
  const supabase = await getSupabaseClient();
  return SearchTopicChunksUtil(supabase, query);
}

// === Podcast request/search actions ===

/** Search for podcasts by name or link (Spotify, Apple, etc). Uses iTunes API - no key needed. */
export async function discoverPodcasts(query: string): Promise<PodcastSearchResult[]> {
  // If it's an Apple Podcasts link, try direct lookup first
  const appleId = extractAppleId(query);
  if (appleId) {
    const result = await lookupPodcast(appleId);
    if (result) return [result];
  }

  return searchPodcastsUtil(query, 8);
}

/** Import a podcast from a search result. Takes the RSS feed URL from the search result. */
export async function requestPodcast(feedUrl: string): Promise<PodcastWithEpisodes> {
  const { userId } = auth();
  if (!userId) {
    throw new Error('Please sign in to add podcasts.');
  }
  console.log(`User ${userId} requesting podcast import: ${feedUrl}`);

  // Use service role to bypass RLS for podcast imports
  const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
  const supabase = serviceKey
    ? await getSupabaseClientWithToken(serviceKey)
    : await getSupabaseClient();

  const podcast = await Ingest({ supabase, podcastUrl: feedUrl, owner: userId });

  // Make the podcast publicly visible
  await supabase
    .from('Podcasts')
  ...
  // Auto-categorize
  try {
    const categorySlug = await CategorizePodcast(supabase, podcast.title, podcast.description || null);
    if (categorySlug) {
      await AssignPodcastCategory(supabase, podcast.id, categorySlug);
    }
  } catch (err) {
...
/** Kick off processing for an episode (requires authentication). */
export async function processEpisodePublic(episodeId: number): Promise<string> {
  const { userId, getToken } = auth();
  if (!userId) {
    throw new Error('Please sign in to process episodes.');
  }
  const supabaseAccessToken = await getToken({ template: 'podverse-supabase' });
  if (!supabaseAccessToken) {
    throw new Error('Unauthorized');
...
/** Kick off processing for all unprocessed episodes in a podcast (requires authentication). */
export async function processPodcastPublic(podcastId: number): Promise<string> {
  const { userId, getToken } = auth();
  if (!userId) {
    throw new Error('Please sign in to process podcasts.');
  }
  const supabaseAccessToken = await getToken({ template: 'podverse-supabase' });
  if (!supabaseAccessToken) {
    throw new Error('Unauthorized');
  }
  console.log(`Processing podcast ${podcastId} for user ${userId}`);
  await inngest.send({
    name: 'process/podcast',
    data: {
