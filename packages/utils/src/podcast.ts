...
type RSSItemWithTranscript = {
  podcastTranscripts?: RSSTranscriptTag[];
};
...
function getTranscriptUrl(item: Parser.Item & RSSItemWithTranscript): string | null {
  const transcripts = item.podcastTranscripts || [];
  for (const transcript of transcripts) {
    const url = normalizeIdentifier(transcript?.$?.url ?? transcript?._ ?? null);
    if (url) {
      return url;
    }
  }
...
function episodeMatchesItem(episode: Episode, item: Parser.Item): boolean {
  const episodeIdentifiers = [
    normalizeIdentifier(episode.guid),
    normalizeIdentifier(episode.url),
    normalizeIdentifier(episode.originalAudioUrl),
  ].filter((value): value is string => Boolean(value));
  if (episodeIdentifiers.length === 0) {
    return false;
  }

  const itemIdentifiers = [
    normalizeIdentifier(item.guid),
    normalizeIdentifier(item.link),
    normalizeIdentifier(item.enclosure?.url),
  ].filter((value): value is string => Boolean(value));
  if (itemIdentifiers.length === 0) {
    return false;
  }

  for (const episodeIdentifier of episodeIdentifiers) {
    if (itemIdentifiers.includes(episodeIdentifier)) {
      return true;
    }
  }
  return false;
...
/** Find a Podcasting 2.0 transcript URL for the given episode in the RSS feed. */
export async function GetEpisodeTranscriptFromFeed(podcastRssUrl: string | null, episode: Episode): Promise<string | null> {
  if (!podcastRssUrl) {
    return null;
  }
  const parserOptions: Parser.ParserOptions<{}, RSSItemWithTranscript> = {
    customFields: {
      item: [['podcast:transcript', 'podcastTranscripts', { keepArray: true }]],
    },
  };
  const parser: Parser<{}, RSSItemWithTranscript> = new Parser({
    ...parserOptions,
  });
  const feed = await parser.parseURL(podcastRssUrl);

  for (const item of feed.items) {
    if (!episodeMatchesItem(episode, item)) {
      continue;
    }
    const transcriptUrl = getTranscriptUrl(item);
    if (transcriptUrl) {
