...
export async function GetPodcasts(
  ...
  // Direct query instead of RPC to avoid dependency on all_podcasts function
  const { data: podcasts, error } = await supabase
    .from('Podcasts')
    .select('*')
    .eq('private', isPrivate ?? false)
    .eq('published', isPublished ?? true)
  ...
  // Attach newestEpisode date for each podcast
  const result: PodcastListEntry[] = [];
  for (const p of podcasts || []) {
    const { data: ep } = await supabase
      ...
      .limit(1);
    result.push({ ...p, newestEpisode: ep?.[0]?.pubDate || p.created_at });
  }
  return result.sort((a, b) => new Date(b.newestEpisode).getTime() - new Date(a.newestEpisode).getTime());
}
...
export async function GetLatestEpisodes({
  ...
  // Direct query instead of RPC to avoid dependency on latest_episodes function
  const { data, error } = await supabase
    .from('Episodes')
    .select('*, Podcasts!inner(slug, title, imageUrl, private, published)')
    .eq('Podcasts.private', false)
    .eq('Podcasts.published', true)
    .not('status', 'is', null)
    .order('pubDate', { ascending: false })
    .limit(limit);
  if (error) {
...
export async function GetPodcastStats(supabase: SupabaseClient): Promise<PodcastStat[]> {
  // Direct query instead of RPC to avoid dependency on podcast_stats function
  const { data: podcasts, error } = await supabase.from('Podcasts').select('*');
  if (error) {
    throw error;
  }
  const stats: PodcastStat[] = [];
  for (const p of podcasts || []) {
    const { count: allEps } = await supabase.from('Episodes').select('*', { count: 'exact', head: true }).eq('podcast', p.id);
    const { data: newestEp } = await supabase.from('Episodes').select('pubDate').eq('podcast', p.id).order('pubDate', { ascending: false }).limit(1);
    stats.push({
      id: p.id, title: p.title, description: p.description, slug: p.slug,
      imageUrl: p.imageUrl, owner: p.owner,
      newest: newestEp?.[0]?.pubDate || '', newestprocessed: '',
      allepisodes: allEps || 0, processed: 0, inprogress: 0, errors: 0,
      process: p.process, private: p.private, uuid: p.uuid,
    });
  }
  return stats;
}
