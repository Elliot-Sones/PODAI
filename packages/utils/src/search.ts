...
export async function Search({
  ...
  // Direct query instead of RPC to avoid dependency on episode_search function
  let episodeQueryBuilder = supabase
    .from('Episodes')
    .select('id, title, description, slug, podcast, imageUrl')
    .or(`title.ilike.%${input}%,description.ilike.%${input}%`)
    .order('pubDate', { ascending: false })
    .limit(10);
  if (podcastSlug && podcast) {
    episodeQueryBuilder = episodeQueryBuilder.eq('podcast', podcast.id);
  }
  const episodeQuery = episodeQueryBuilder;
